...
class TutorialVariantA(Tutorial):
	def __init__(self, main_window, app, command_callback, metrics):
		super().__init__(main_window, app, command_callback, metrics)
		self._starting_directory = None
	def start(self):
		self._starting_directory = self._pane.get_path()
		super().start()
	def _get_steps(self):
		goto_directory = basename(self._get_directory_for_goto())
